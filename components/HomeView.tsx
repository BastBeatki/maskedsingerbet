    ...
    <div className="min-h-screen flex flex-col items-center p-4 sm:p-6 lg:p-8">
      <div className="w-full max-w-6xl mx-auto">
        <div className="text-center mb-12">
            <h1 className="text-5xl sm:text-7xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary mb-4">
            The Masked Singer Tipper
            </h1>
            <p className="text-xl text-text-secondary">Willkommen zurück! Starte eine neue Season oder sieh dir eine alte an.</p>
        </div>
        
        <Card className="mb-10">
            <h2 className="text-2xl font-bold mb-4 text-center">Neue Season starten</h2>
            <form onSubmit={handleAddSeason} className="flex flex-col sm:flex-row gap-4 max-w-lg mx-auto">
                <Input
                    type="text"
                    value={newSeasonName}
                    onChange={(e) => setNewSeasonName(e.target.value)}
                    placeholder="Name der neuen Season"
                    className="flex-grow"
                    required
                />
        ...
        {appState.seasons.length > 0 && (
            <div className="mb-10">
                <h2 className="text-3xl font-bold mb-6">Vergangene Seasons</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {appState.seasons.map(season => (
                        <div key={season.id} className="group relative rounded-xl overflow-hidden cursor-pointer" onClick={() => onNavigate('game', season.id)}>
                            <img 
